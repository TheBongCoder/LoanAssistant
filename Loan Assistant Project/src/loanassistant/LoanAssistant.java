...
public class LoanAssistant extends JFrame {
	...
	{	
		...
		GridBagConstraints gridConstraints;
			...
			//*This code also adds listeners for each button. 
	...
	private void computeButtonActionPerformed(ActionEvent e)
	{
		...
		{
			JOptionPane.showConfirmDialog(null, "Invalid or empty Loan Balance entry.\nPlease correct.", "Balance Input Error",JOptionPane.
		...
		{
			mul= Math.pow(1+monthlyInterest, months);
			payment = balance*monthlyInterest*mul/(mul-1);
		}
		// DecimalFormat() is use assign decimal points to the results.
		...
		else
		{
			...
			if(validateDecimalNumber(paymentTextField))
			{
				...
				{
					if(JOptionPane.showConfirmDialog(null, "Mininum payment must be ₹"
		...
		//show analysis
			...
			analysisTextArea.append("\n"+"Interest: " +"\t"+ new DecimalFormat("0.00").format(interest)+"%");
	...
	private boolean validateDecimalNumber(JTextField tf)
	{
		...
		//valid decimal number with only digits and a decimal point
		String s = tf.getText().trim(); //Textfield stored in string
		boolean hasDecimal= false; // each charcter in this string is evaluted
		boolean valid= true;	//only allows no. & single decimal point.
							//if one decimal & no. found VALID is TRUE and proceed
					// if VALID is FALSE; invalid char & empty String 
		if(s.length()==0)
		{
			valid=false;
		}
